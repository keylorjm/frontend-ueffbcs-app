  ...
  return authService.isAuthenticated$.pipe(
    switchMap((isAuthenticated) => {
      ...
      if (isAuthenticated) {
        ...
        if (role === 'admin') return of(router.createUrlTree(['/app/dashboard-admin']));
      ...
      if (hasToken) {
        return authService.ensureUserLoaded().pipe(
          map((ok): boolean | UrlTree => {
            ...
            const role = (authService.getrole() ?? '').toLowerCase();
            if (role === 'profesor') return true;
            if (role === 'admin') return router.createUrlTree(['/app/dashboard-admin']);
